...
def main():
	#Get args['QuantITkit'] and if there is a standard curve
	...
	ascFiles = glob.glob('C:\\Tecan\\asc\\*')
...
#		plt.title('%s standard curve %s' % (args['QuantITkit'], date))
#		plt.xlabel('Raw data')
#		plt.ylabel('Concentration [ng/ul]')
#		plt.savefig('C:\\Tecan\\Variables\\StdCurves\\%sStdCurve-%s.png' % (args['QuantITkit'], date))
#		plt.close()

	elif args['stdCurve'] == 'Yes':
		#read in slope and blank
		with open(storedStdCurve, 'r') as file:
			stdDate = next(file).strip()
...
#		plt.close()
	...
	resultFile = 'C:\\Tecan\\MagellanConcentrations\\' + args['QuantITkit'] + date + '.csv'
	concData = ['']*96
